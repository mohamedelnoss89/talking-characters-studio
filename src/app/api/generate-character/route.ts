...
export async function POST(req: NextRequest) {
  ...
  // Parse body first — if invalid, return a normal JSON error (no streaming needed).
  let body: CharacterRequestBody;
  try {
    body = (await req.json()) as CharacterRequestBody;
  } catch {
  ...
  // Streaming response مع heartbeat عشان نمنع البروكسي/الجيتواي إنه يقفل الاتصال.
  // الـ JSON parser بيتجاهل الـ whitespace اللي في الأول، فمفيش حاجة تتغير على الـ client.
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      // Heartbeat: ابعت سطر فاضي كل 2 ثانية عشان الاتصال يفضل حيّ
      ...
      try {
        ...
        try {
          ...
          safeEnqueue(JSON.stringify(errResp));
          return;
        }
        ...
        safeEnqueue(JSON.stringify(finalResp));
      } catch (err: any) {
        console.error("[generate-character] error:", err);
        const errResp = {
          success: false,
          error: err?.message ? String(err.message) : "Character generation failed",
        };
        safeEnqueue(JSON.stringify(errResp));
      } finally {
    ...
    cancel() {
      // client aborted — nothing to clean here, heartbeat cleared on next tick
    },
  });
  ...
  return new Response(stream, {
    ...
    headers: {
      ...
      "X-Accel-Buffering": "no", // عشان nginx ما ي缓冲ش الرد
